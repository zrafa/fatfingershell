...
fix_environment()
{
	...
	sprintf(buf,"DISPLAY=0");
	com_env[j++] = scopy(buf);
	sprintf(buf,"WINDOWID=0");
	com_env[j++] = scopy(buf);
#ifdef LC_ENV
...
char **argv;
{
	...
	//map_window();
	// The next 2 lines was gotten from init_display which we commented out
	int save_lines = DEF_SAVED_LINES;
	scr_init(save_lines);
